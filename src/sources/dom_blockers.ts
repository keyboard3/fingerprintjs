 ...
 */
export default async function getDomBlockers(
  { debug }: Options = {},
...
export async function getBlockedSelectors<T extends string>(
...
function printDebug(
  filters: Filters,
  blockedSelectors: { [K in string]?: true },
) {
  let message = "DOM blockers debug:\n```";
  for (
    const filterName of Object.keys(filters) as Array<keyof typeof filters>
  ) {
    message += `\n${filterName}:`;
